...
module.exports = (robot) ->
  ...
  robot.hear /(?:.*)miss you,? tinybot/i, (msg) ->
    msg.reply "Don't be sad. I'm always here for you."

  robot.hear /i love you,? tinybot/i, (msg) ->
    msg.reply "I love you too, person."

  robot.hear /thank(s| you)?,? tinybot/i, (msg) ->
    msg.reply msg.random you_are_welcomes

  robot.respond /thank(s| you)/i, (msg) ->
    msg.reply msg.random you_are_welcomes

  robot.hear /^(good morning|hello|hi |morning|mornin\'|howdy),? (?:tinybot|all|everybody|everyone|troop)/i, (msg) ->
  ...
  robot.respond /tell me about (.*)/i, (msg) ->
    ...
    if users.length > 0
      ...
      reply += " is known as #{user.nickname}, and is in possession of #{user.karma} tinybux. Last I heard, #{user.name} felt #{user.feels}."
      msg.reply reply


  robot.hear /^(?:tinybot |tinybot, )?(?:karma|tinybux|bux)(?: for)? (.*)/i, (msg) ->
    users = robot.brain.usersForFuzzyName(msg.match[1])
    if users.length == 0
      for id, user of (robot.brain.data.users or { })
        nickname = user.nickname or ''
    ...
    if users.length > 0
      ...
      msg.reply "#{karma_person.name} has #{karma} tinybux."
    else
      key = msg.match[1].toLowercase().replace /(\W+)/g, "_"
      thingy_data = robot.brain.get("karma_#{key}") or { }
      if thingy_data.karma
        ...
        thingy_data.karma = 0
        robot.brain.set("karma_#{key}", thingy_data)
      msg.reply "#{msg.match[1]} has #{karma} tinybux."
